...
export class SlackClient {
    ...
    async getReply(
        ...
        try {
            ...
            lastMessageText = unescapeHtml(lastMessageText)
            let nextCompletion = lastMessageText.replace(END_TEXT, '').replace(prevCompletion, '')
            if (lastMessage && !lastMessage.text.endsWith(END_TEXT)) {
                callback && callback(nextCompletion, STATUS.STOP);
                return lastMessage.text;
            }
            nextCompletion = nextCompletion.replace(/[ \n]+$/, '')
            ...
            return await this.getReply(
                {
                    count: count + 1,
                    prevCompletion: lastMessageText.replace(END_TEXT, '').replace(/[ \n]+$/, ''),
                },
                callback,
            );
        } catch (error) {
